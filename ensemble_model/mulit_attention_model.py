...
class AttentionMoEModel(nn.Module):
    ...
    def forward(self, inputs_bert, inputs_codebert):
        # 得到 [batch_size, hidden_size]
        outputs_bert = self.base_model1(inputs_bert)
        outputs_codebert = self.base_model2(inputs_codebert)

        # 拼接为 [batch_size, seq_len=2, hidden_size]
        combined = torch.stack([outputs_bert, outputs_codebert], dim=1)

        # 注意力层输入格式：batch_first=True => [batch_size, seq_len, hidden_size]
        attn_output, attn_weights = self.attention(combined, combined, combined)

        # 例如取第一个 token 的输出作为聚合表示，也可以改成 mean pooling
